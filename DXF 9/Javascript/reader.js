...
let ob4 = {
            ...
            switch(this.key){
                case('  0'):
                    switch(this.value){
                        case('ENDSEC'):
                            //start parse
                            await this.handleVar(output, undefined, await this.parse("  0", 'SECTION'));
                        break;
                        case('ENDTAB'):
                            await this.handleVar(output, undefined, await this.parse("  0", 'TABLE'));
                        break;
                        case('ENDBLK'):
                            //await this.handleVar(output, undefined, await this.parse("  0", 'BLOCK'));
                        break;
                        case('SEQEND'):
                            await this.handleVar(output, undefined, await this.parse("  0", 'INSERT'));
                        break;
                        case('BLOCK_RECORD'):
                            Object.assign(read,readread);
                ...
                case('102'):
                    if(this.value !== '}'){
                        ...
                        await this.handleVar(readread, undefined, await this.parse("102", '{}'));
                    };
                break;
                case('1002'):
                    if(this.value !== '}'){
                        ...
                        await this.handleVar(readread, undefined, await this.parse("1002", '{}'));
                    };
    ...
    async handleVar(output, to, value) {
        ...
        if (typeof output[to] !== 'undefined') {
            ...
            if(to == ' 42'){
               if(Array.isArray(output[" 10"])){
                   if(!Array.isArray(output[" 42"])){output[' 42'] = [,output[to]];}
        ...
        ob4.read(text).then((value) => {file = value; connected(file)}); //was init om te tekenen
               // connected

           //console.log( file);

            //const result = parseData(text.split('\r\n').slice(0, -1));
            //console.log(result);
        }
